...
export function duoAffinityMatrix(ids: (number | null)[]): number[][] {
    const matrix: number[][] = [];
    for (let i = 0; i < ids.length; i++) {
        const row: number[] = [];
        for (let j = 0; j < ids.length; j++) {
            row.push(duoAffinity(ids[i], ids[j]) ?? 0);
        }
        matrix.push(row);
    }
    return matrix;
...
export function trioAffinityMatrix(target: number | null, ids: number[]): number[][] {
    const matrix: number[][] = [];
    for (let i = 0; i < ids.length; i++) {
        const row: number[] = [];
        for (let j = 0; j < ids.length; j++) {
            row.push(trioAffinity(target, ids[i], ids[j]) ?? 0);
        }
        matrix.push(row);
    }
    return matrix;
